...
FTS5 인덱스 + 스니펫 추출 = 90%+ 절감 / 10배 이상 (1.5K)
...
| 기능 | 설명 |
|------|------|
| 🔍 **빠른 검색** | FTS5 기반 전문 검색으로 90%+ 절감 / 10배 이상 (실측 93~98.9% / 14~90배) |
...
이 시스템은 효율적인 메모리 검색을 제공하지만, AI Agent가 **과거 기록에만 의존하면 현재의 생각과 판단을 잃을 수 있습니다.**
...
- **현재 상황과 컨텍스트**를 우선 고려
- 과거 기록이 항상 옳은 것은 아님
- **스스로 생각하고 판단**하는 능력 유지
- 메모리는 보조 도구, 의사결정의 전부가 아님

**기억:** 검색이 빠르다고 해서 모든 결정을 메모리에 의존하면, 정체성이 흐려지고 기계적 반복에 빠질 수 있습니다.

**균형:** 메모리 참조 + 현재 사고 + 새로운 통찰 = 살아있는 AI Agent
...
> "90% 기대했는데 실측 95.7%. 기대 이상. 속도 15배 빨라짐."
> - 테스트 Agent, 2026-03-05

...
OpenClawユーザーなら一行でインストール:

```bash
clawhub install eidolon-search
```

AIエージェントが自動的に認識して使用します。「メモリを検索して」と言うだけで動作します。
...
依存関係なし。Python 3.6+とSQLite3だけで動作します。
...
```
scripts/build-index.py  # インデキシング
scripts/search.py       # 検索
```
